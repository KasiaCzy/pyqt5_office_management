...
class UpdateEventWindow(QWidget):
    ...
    def create_widgets(self):
        ...
        event = self.cursor.execute(event_query, (self.selected_event_id,)).fetchone()
        self.events_box = QComboBox()
        self.events_box.addItems(['holiday', 'work day'])
        index = self.events_box.findText(event[1], Qt.MatchFixedString)
        self.events_box.setCurrentIndex(index)
        self.event_date = QDateEdit()
        self.event_date.setFont(QFont('', 10))
        dt = datetime.strptime(event[2], "%d.%m.%Y")
        ...
        self.note_entry.setText(event[3])
